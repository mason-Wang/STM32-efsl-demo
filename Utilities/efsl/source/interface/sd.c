...
/////////////////////////////////////////////////////////////
	...
	if (resp == 1)	   // SDHC
	{
		...
		if (ocr[2] == 0x01 && ocr[3] == 0xAA) 
		{
			...
			i = 50000;
			while (i-- && sd_Command1(iface, 41, (1 << 14), 0));	
		}	
	}
	else
	{
...
void sd_Command(hwInterface *iface,euint8 cmd, euint16 paramx, euint16 paramy)
{
	...
	if (cmd == 0x08)
		n = 0x87;															               
	if_spiSend(iface,n); /* Checksum (should be only valid for first command (0) */
	if_spiSend(iface,0xff); /* eat empty command - response */
}

euint8 sd_Command1(hwInterface *iface,euint8 cmd, euint16 paramx, euint16 paramy)
{
	euint32 i;
	euint8 resp;
	...
	if_spiSend(iface,(0xc0+cmd)&0x7f);
	if_spiSend(iface,(euint8) (paramx >> 8)); /* MSB of parameter x */
	if_spiSend(iface,(euint8) (paramx)); /* LSB of parameter x */
	if_spiSend(iface,(euint8) (paramy >> 8)); /* MSB of parameter y */
	if_spiSend(iface,(euint8) (paramy)); /* LSB of parameter y */
																               
	if_spiSend(iface,0x01); /* Checksum (should be only valid for first command (0) */
	if_spiSend(iface,0xff); /* eat empty command - response */
